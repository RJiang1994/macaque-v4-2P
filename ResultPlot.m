...
for i = 1:292
    for j = 1:292
        DistanceA(i,j) = 850/512 * sqrt( (CxA(i)-CxA(j))^2 + (CyA(i)-CyA(j))^2);
...
for i = 1:243
    for j = 1:243
        DistanceB(i,j) = 850/512 * sqrt( (CxB(i)-CxB(j))^2 + (CyB(i)-CyB(j))^2);
...
%%  Tuning correlation vs Distance
